...
void print_mat_accuracy(Matrix* x , Matrix* v , double* arr ,double* y, Matrix* acc ,int rank){
    int i;
Matrix temp;
temp.width =np+nn;
temp.height = 1 ;
...
void print_rescale_to_standard(double* arr,int size){
int i;double min;double max;double sum;
sum=0;
min = arr[0];
max= arr[0];
...
void fitness_accuracy(Matrix* x , Matrix* v , double* arr ,double* y, Matrix* acc, double* Host_c,int height,int rank){
     int i;    int temp_i=0;
Matrix temp;
temp.width =np+nn;
temp.height = height ;
temp.elements = (double*)malloc(temp.height*temp.width*sizeof(double));
    for(temp_i=0;temp_i<temp.height;temp_i++){
for(i=0;i<temp.width;i++){
    temp.elements[temp.width*temp_i+i] = arr[temp.width*temp_i+i]*y[i];
}
}
Matrix C;
 C.height = x->height;
...
Matrix P;
 P.height = temp.height;
    ...
    MatMul(temp,C,P,rank);
    double best_acc = 0;
    for(temp_i=0;temp_i<P.height;temp_i++){
    double val =0;
    for(i=0;i<P.width;i++){
if((acc->elements[i]*P.elements[P.width*temp_i+i]>=0)){
val = val+1.0;
}
    }
    val = (val/(P.width*1.0));
...
Host_c[temp_i] = val;
if(Host_c[temp_i]>best_acc){
    best_acc = Host_c[temp_i];
}
}
printf("best_acc is %f\n", best_acc);
    free(temp.elements);
    free(C.elements);
    free(P.elements);
    return ;
...
printf("rank is %d\n", rank);
    FILE * file_p = fopen("../adult_dataset/adult_xp.csv","r");
    FILE * file_n = fopen("../adult_dataset/adult_xn.csv","r");
    FILE * file_v = fopen("../adult_dataset/adult_vt.csv","r");
...
for(ii = 0 ;ii < A.width ; ii ++){
    A.elements[row_num*A.width + ii] = alpha.values[ii] ;
}
...
row_num++;
  ...
  /*  temp=0;
    for(i=0; i<A.height; i++)
    {
        temp=0;
        for(j=0; j<np; j++)
            temp+=A.elements[i*A.width + j];
        printf("%f\t", temp);
        temp=0;
        for(j=0; j<nn; j++)
            temp+=A.elements[i*A.width + np + j];
        printf("%f\t", temp);
        printf("\n");
    }*/
    /// A is created
    ...
    Matrix C,D;
    int a1, a2, b1, b2;
    a1 = (np+nn); /* Height of A */
    a2 = d; /* Width of A */
    b1 = a2;    /* Height of B */
    b2 = a2;//atoi(argv[3]); /* Width of B */
   ...
   // cudaSetDevice(1);
    ...
    for(int i=0;i<D.height; i++){
       ...
       // printf("\n");
        ...
        m = string_arg_m;
        print_rescale_to_standard(Host_c,m);
        bool* present;
        present = (bool*)malloc(m*sizeof(bool));
        double avg_F;
        double sum_present_F=0;
...
///code for doing crossover using pool
//print_accuracy(max_alpha_A,y->elements,x->elements,v,v_size,(np+nn),d);
print_mat_accuracy(x,v_t,max_alpha_A,y->elements, acc,rank);
free(max_alpha_A);
...
n = A.height; // number of matrix rows/cols
            m = A.width;
            get_host(D,A,Host_c,n,m);*/
        
///filter the values more than average
 fitness_accuracy(x,v_t,A.elements,y->elements, acc,Host_c,A.height,rank);
...
///code for doing crossover using pool
//print_accuracy(max_alpha_A,y->elements,x->elements,v,v_size,(np+nn),d);
//mat_accuracy(x,v_t,max_alpha_A,y->elements, acc);
free(max_alpha_A);
