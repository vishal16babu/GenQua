...
extern "C" void MatMul(Matrix AA, Matrix B, Matrix C, int dev_no)
{  
int nDevices;
...
__global__ void MatMulKernel(Matrix AA, Matrix B, Matrix C)
{
    double Cvalue = 0;
    ...
    if(row>AA.height||col>B.width)
        return;
    for (int e = 0; e < AA.width; ++e)
        Cvalue += (AA.elements[row * AA.width + e]) * (B.elements[e * B.width + col]);
